  ...
  return (
    <div className="card flex flex-col h-[520px]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h4 className="text-lg font-bold text-gray-900">Hoạt động thời gian thực</h4>
          <p className="text-xs text-gray-500 font-medium">Theo dõi các hành động gần đây của khách truy cập</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1">
        {feed.length === 0 ? (
          ...
          feed.map((item) => (
            <div
              key={item.id}
              className="flex gap-3 text-xs p-3 rounded-lg bg-white/50 border border-gray-100 hover:bg-gray-50 transition-all duration-200"
            >
              {/* Type Icon Indicator */}
              <div className="flex-shrink-0 self-start mt-0.5">
                {item.type === "pageview" && (
                  <div className="p-1.5 rounded-md bg-blue-500/10 text-blue-600 border border-blue-500/20">
                ...
                {item.type === "error" && (
                  <div className="p-1.5 rounded-md bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <div className="flex-1 overflow-hidden space-y-1">
                ...
                {item.description && (
                  <p className="text-gray-600 font-mono text-[10px] break-all bg-gray-50 p-1.5 rounded border border-gray-100">
                    {item.description}
                  </p>
                )}
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 items-center text-[10px] text-gray-500 font-medium">
                  ...
                  {/* Device */}
                  <span className="flex items-center gap-1 border-l border-gray-250 pl-3">
                    <DeviceIcon type={item.device} className="w-3.5 h-3.5 text-gray-400" />
